(function() {
  var CUSTOM_EMBED_WARNING, CustomEmbedPlayer, DEFAULT_ERROR, DailymotionPlayer, EmbedPlayer, FilePlayer, GoogleDrivePlayer, GoogleDriveYouTubePlayer, HLSPlayer, ImgurPlayer, LivestreamPlayer, MixerPlayer, Player, PlayerJSPlayer, RTMPPlayer, SmashcastPlayer, SoundCloudPlayer, StreamablePlayer, TYPE_MAP, TwitchPlayer, UstreamPlayer, VideoJSPlayer, VimeoPlayer, WebtorrentPlayer, YouTubePlayer, codecToMimeType, genParam, getSourceLabel, sortSources,
      ...
      return waitUntilDefined(window, 'playerjs', (function(_this) {
        return function() {
          ...
          if (data.type !== "wt") {
            ...
            iframe = $('<video/>').addClass('video-js vjs-default-skin embed-responsive-item').attr(attrs);
            removeOld(iframe);
          }
  ...
  window.MixerPlayer = MixerPlayer = (function(superClass) {
    extend(MixerPlayer, superClass);

    function MixerPlayer(data) {
      if (!(this instanceof MixerPlayer)) {
        return new MixerPlayer(data);
      }
      this.load(data);
    }

    MixerPlayer.prototype.load = function(data) {
      data.meta.embed = {
        src: "https://mixer.com/embed/player/" + data.meta.mixer.channelToken,
        tag: 'iframe'
      };
      return MixerPlayer.__super__.load.call(this, data);
    };
  ...
  window.WebtorrentPlayer = WebtorrentPlayer = (function(superClass) {
    extend(WebtorrentPlayer, superClass);

    function WebtorrentPlayer(data) {
      if (!(this instanceof WebtorrentPlayer)) {
        return new WebtorrentPlayer(data);
      }
      this.client = new WebTorrent();
      WebtorrentPlayer.__super__.constructor.call(this, data);
    }

    WebtorrentPlayer.prototype.load = function(data) {
      var k;
      data.meta.playerjs = {
        magnet: "" + data.id
      };
      WebtorrentPlayer.__super__.load.call(this, data);
      return k = this.client.add(data.id, function (torrent) {
